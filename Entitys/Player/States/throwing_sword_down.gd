...
func physics_update(delta: float) -> void:
	...
	player.velocity.x = lerpf(player.velocity.x, move_direction.x * player.sword_throw_down_jump_speed, delta * player.sword_throw_down_player_deaccel)
	player.velocity.z = lerpf(player.velocity.z, move_direction.z * player.sword_throw_down_jump_speed, delta * player.sword_throw_down_player_deaccel)
	# player.velocity.y = lerpf(player.velocity.y, 0, delta * player.sword_throw_down_player_deaccel)
...
func launch_sword():
	...
	throw_dirction = Vector3.DOWN.rotated(Vector3.UP, player.rotation.y)
